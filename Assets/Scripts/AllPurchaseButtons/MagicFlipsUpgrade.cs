...
public class MagicFlipsUpgrade : MonoBehaviour
{
    public GameObject realMagicUpgrade;
    public GameObject fakeMagicUpgrade;
    public TextMeshProUGUI magicUpgradeText;
    public TextMeshProUGUI fakeMagicUpgradeText;
    public static float currentCoins;
    public static float magicUpgradeCost = 7500000000000000000000f;
    public static bool turnoffMagicUpgrade = false;
    public static int magicUpgradeCount;
    ...
    public void Update()
    {
        ...
        if (Upgrade12.upgrade12Count == 0)
        {
            ...
            fakeMagicUpgrade.SetActive(true);
            realMagicUpgrade.SetActive(false);
            fakeMagicUpgradeText.color = Color.white;
            fakeMagicUpgradeText.GetComponent<TextMeshProUGUI>().text = "???";
            buttonName.GetComponent<TextMeshProUGUI>().text = "????? ?????";
            buttonName.color = Color.white;
        }
        if (Upgrade12.upgrade12Count > 0)
        {
            buttonName.GetComponent<TextMeshProUGUI>().text = LocalizationStrings.magicFlipsName;
            buttonIcon.SetActive(true);
            coverOver.SetActive(false);
            magicUpgradeText.color = Color.green;
            fakeMagicUpgradeText.color = Color.red;

            magicUpgradeText.text = FormatCoins(magicUpgradeCost);
            fakeMagicUpgradeText.text = FormatCoins(magicUpgradeCost);
          

            if (currentCoins >= magicUpgradeCost)
            {
                realMagicUpgrade.SetActive(true);
                fakeMagicUpgrade.SetActive(false);
            }
            if (currentCoins < magicUpgradeCost)
            {
                realMagicUpgrade.SetActive(false);
            ...
            if (turnoffMagicUpgrade == true)
            {
                realMagicUpgrade.SetActive(true);
                fakeMagicUpgrade.SetActive(false);
                turnoffMagicUpgrade = false;
            }
        }
