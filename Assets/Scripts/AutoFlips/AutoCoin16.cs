...
public class AutoCoin17 : MonoBehaviour
{
    ...
    public VideoPlayer videoPlayer17;
    public GameObject coinFlipper17;
    public GameObject transformPopUp;
    public static int ascendedAuto16 = 0;
    public TestingNewAnim newFlipScript;

    public void Awake()
    {
        mainButtonClick1 = videoPlayer17.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto16 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer17.Play(); }
        else { newFlipScript.Flip(17); }
    ...
    {
        if (FlipperMute.flipperMute16 == 0)
        {
