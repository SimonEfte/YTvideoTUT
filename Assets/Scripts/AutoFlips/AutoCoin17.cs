...
public class AutoCoin16 : MonoBehaviour
{
    public static int ascendedAuto17 = 0;
    ...
    public VideoPlayer videoPlayer16;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer16.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper16.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto17 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer16.Play(); }
        else { newFlipScript.Flip(16); }
    ...
    {
        if (FlipperMute.flipperMute17 == 0)
        {
