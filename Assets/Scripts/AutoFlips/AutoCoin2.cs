...
public class AutoCoin2 : MonoBehaviour
{
    public static int ascendedAuto2 = 0;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer2.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper2.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto2 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer2.Play(); }
        else { newFlipScript.Flip(2); }
