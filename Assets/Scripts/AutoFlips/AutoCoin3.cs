...
public class AutoCoin3 : MonoBehaviour
{
    public static int ascendedAuto3 = 0;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer3.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper3.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto3 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer3.Play(); }
        else { newFlipScript.Flip(3); }
