...
public class AutoCoin5 : MonoBehaviour
{
    public static int ascendedAuto5 = 0;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer5.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper5.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto5 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer5.Play(); }
        else { newFlipScript.Flip(5); }
