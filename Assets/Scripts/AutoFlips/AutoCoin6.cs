...
public class AutoCoin6 : MonoBehaviour
{
    public static int ascendedAuto6 = 0;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer6.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper6.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto6 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer6.Play(); }
        else { newFlipScript.Flip(6); }
