...
public class AutoCoin7 : MonoBehaviour
{
    public static int ascendedAuto7 = 0;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer7.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper7.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto7 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer7.Play(); }
        else { newFlipScript.Flip(7); }
