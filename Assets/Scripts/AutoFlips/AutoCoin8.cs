...
public class AutoCoin8 : MonoBehaviour
{
    public static int ascendedAuto8 = 0;
    ...
    public VideoPlayer videoPlayer8;
    ...
    public void Awake()
    {
        mainButtonClick1 = videoPlayer8.GetComponent<MainButtonClick>();
        mainButtonClick1 = coinFlipper8.GetComponent<MainButtonClick>();
    }

    void Update()
    {
        if (GlobalCoinMaker.coinsPerFlipUpgrades == 0) { CreatingCoin = false; }
        if (CreatingCoin == false && ascendedAuto8 == 1)
        {
            CreatingCoin = true;
    ...
    {
        if (LoadSaves.isMobileBuild == false) { videoPlayer8.Play(); }
        else { newFlipScript.Flip(8); }
