...
public class SpecialHoverOver : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    public GameObject textBar;
    public TextMeshProUGUI textPrice;
    public TextMeshProUGUI upgradecCoinCount;
    ...
    void Update()
    {
        upgradecCoinCount.GetComponent<TextMeshProUGUI>().text = LocalizationStrings.purchased + GoldenFlipsButton.goldenCoinsUpgradeCount;
        if (MainButtonClick.numberOfDuplicationUpgrades < 20)
        {
            textPrice.color = Color.red;
            textPrice.GetComponent<TextMeshProUGUI>().text = LocalizationStrings.needs21Flippers;
        }

        if (MainButtonClick.numberOfDuplicationUpgrades == 20)
        {
            if (GOLDENFLIPS.currentCoins >= GOLDENFLIPS.specialCoinsButtonCost)
            {
                textPrice.color = Color.green;
                textPrice.text = FormatCoins(GOLDENFLIPS.specialCoinsButtonCost);
    ...
    public void SetFrame()
    {
        ...
        {
            textBar.transform.localScale = new Vector2(SetInfoAllButtons.scale - 0.08f, SetInfoAllButtons.scale - 0.08f);
            textBar.transform.position = new Vector2(SetInfoAllButtons.xpos - 0.37f, SetInfoAllButtons.ypos + 0.16f);
        }
        else
        {
