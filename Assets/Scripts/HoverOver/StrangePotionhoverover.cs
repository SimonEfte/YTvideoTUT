...
public class StrangePotionhoverover : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update()
    {
        if (StrangePotion.currentCoins >= StrangePotion.potionCost)
        {
            textPrice.color = Color.green;
            textPrice.text = FormatCoins(StrangePotion.potionCost);
    ...
    public void SetFrame()
    {
        ...
        if (PurchaseLog.strangePotiongPlussAmount >= 1000000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.strangePotiongPlussAmount / 1000000000000D).ToString("0.##") + LocalizationStrings.plussTrillionCoins;
        }
        else if (PurchaseLog.strangePotiongPlussAmount >= 1000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.strangePotiongPlussAmount / 1000000000D).ToString("0.##") + LocalizationStrings.plussBillionCoins;
        }

        if (LoadSaves.isMobileBuild == true)
        {
            textBar.transform.localScale = new Vector2(SetInfoAllButtons.scale, SetInfoAllButtons.scale);
            textBar.transform.position = new Vector2(SetInfoAllButtons.xpos, SetInfoAllButtons.ypos - 2.15f);
        }
        else
        {
