...
public class TreasureHoverOver : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update()
    {
        textPrice.text = FormatCoins(TreasureChest.treasureCost);

        if (TreasureChest.currentCoins >= TreasureChest.treasureCost)
        {
    ...
    public void SetFrame()
    {
        ...
        if (PurchaseLog.treasureChestPluss >= 1000000000000000000000f)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.treasureChestPluss / 1000000000000000000000D).ToString("0.##") + LocalizationStrings.plussSextillionCoins;
        }
        else if (PurchaseLog.treasureChestPluss >= 1000000000000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.treasureChestPluss / 1000000000000000000D).ToString("0.##") + LocalizationStrings.plussQuintillionCoins;
        }
    ...
    {
        if (LoadSaves.isMobileBuild == false) { textBar.SetActive(false); }
    }
}
