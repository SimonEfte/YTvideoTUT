...
public class explosiveHoverOver : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update()
    {
        if (ExplosiveFlips.currentCoins >= ExplosiveFlips.explosiveCost)
        {
            textPrice.color = Color.green;
            textPrice.text = FormatCoins(ExplosiveFlips.explosiveCost);
    ...
    public void SetFrame() 
    {
        ...
        if (PurchaseLog.explosiveFlipsPlussAmount >= 1000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.explosiveFlipsPlussAmount / 1000000D).ToString("0.##") + LocalizationStrings.plussMillionCoins;
        }
        else if (PurchaseLog.explosiveFlipsPlussAmount >= 1000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.explosiveFlipsPlussAmount / 1000000000D).ToString("0.##") + LocalizationStrings.plussBillionCoins;
        }

        if (LoadSaves.isMobileBuild == true)
        {
            textBar.transform.localScale = new Vector2(SetInfoAllButtons.scale, SetInfoAllButtons.scale);
            textBar.transform.position = new Vector2(SetInfoAllButtons.xpos, SetInfoAllButtons.ypos - 1.5f);
        }
        else
        {
