...
public class masterFlipperhoverover : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update()
    {
        if (MasterFlipper.currentCoins >= MasterFlipper.MasterFlipsCost)
        {
            textPrice.color = Color.green;
            textPrice.text = FormatCoins(MasterFlipper.MasterFlipsCost);
    ...
    public void SetFrame()
    {
        ...
        if (PurchaseLog.masterFlipsPlussAmount >= 1000000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.masterFlipsPlussAmount / 1000000000000D).ToString("0.##") + LocalizationStrings.plussTrillionCoins;
        }
        else if (PurchaseLog.masterFlipsPlussAmount >= 1000000000)
        {
            textPlussAmount.GetComponent<TextMeshProUGUI>().text = "+" + (PurchaseLog.masterFlipsPlussAmount / 1000000000D).ToString("0.##") + LocalizationStrings.plussBillionCoins;
        }

        if (LoadSaves.isMobileBuild == true)
        {
            textBar.transform.localScale = new Vector2(SetInfoAllButtons.scale, SetInfoAllButtons.scale);
            textBar.transform.position = new Vector2(SetInfoAllButtons.xpos, SetInfoAllButtons.ypos - 2.06f);
        }
        else
        {
