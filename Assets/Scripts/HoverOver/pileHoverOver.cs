...
public class pileHoverOver : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void Update()
    {
        ...
        {
            pileCost.color = Color.red;
            pileCost.GetComponent<TextMeshProUGUI>().text = LocalizationStrings.maxedOut;
        }
        else
        {
            if (PileOfCoins.currentCoins >= PileOfCoins.pileOfCoinsCost)
            {
                pileCost.color = Color.green;

                pileCost.text = FormatCoins(PileOfCoins.pileOfCoinsCost);
            }
